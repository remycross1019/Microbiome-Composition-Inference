...
def run_neural_network(x, y, hidden, str, s_thresh, t_thresh):
    wxs_train, wxs_test, wgs_train, wgs_test = train_test_split(x, y, random_state=1)
    wxs_test.to_csv('/Users/remycross/Desktop/WXS to WGS/KerasModel/FinalResults2/WXS_Test_{}_{}_{}.csv'.format(s_thresh, t_thresh,
                                                                                                   str))
    wgs_test.to_csv('/Users/remycross/Desktop/WXS to WGS/KerasModel/FinalResults2/WGS_Test_{}_{}_{}.csv'.format(s_thresh, t_thresh,
                                                                                                   str))
    testdist = []
    for i in range(len(wgs_test)):
        testdist.append(
            K.square(weighted_unifrac(wxs_test.to_numpy()[i], wgs_test.to_numpy()[i], otu_ids, data_tree, validate=False,
                                      normalized=True)))
    testdist = pd.DataFrame(testdist, columns=[0])
    testdist.to_csv(
        '/Users/remycross/Desktop/WXS to WGS/KerasModel/FinalResults2/Test_{}_{}_{}.csv'.format(s_thresh, t_thresh,
                                                                                                   str))
    print('done')
    model = Sequential()
    ...
    print(wxs_train.shape)
    print(wgs_train.shape)
    history = model.fit(wxs_train.to_numpy(), wgs_train.to_numpy(), epochs=1000, batch_size=20)
    ...
    wgs_pred.to_csv(
        '/Users/remycross/Desktop/WXS to WGS/KerasModel/FinalResults2/Predicted_{}_{}_{}.csv'.format(s_thresh, t_thresh,
                                                                                                    str))
    unidist.to_csv(
        '/Users/remycross/Desktop/WXS to WGS/KerasModel/FinalResults2/Unifrac_{}_{}_{}.csv'.format(s_thresh, t_thresh,
                                                                                                  str))
    history=pd.DataFrame(history.history['loss'])
    history.to_csv('/Users/remycross/Desktop/WXS to WGS/KerasModel/FinalResults2/History_{}_{}_{}.csv'.format(s_thresh, t_thresh,
                                                                                                    str))
...
if __name__ == "__main__":
    ...
    layers = [float(x) for x in d['layers'].split('_')]
    wxs, wgs = create_data()
    ...
    weighted_unifrac_loss(np.asarray(wxs),np.asarray(wgs))
    run_neural_network(wxs, wgs, layers, layers_string, sample_threshold, taxa_threshold)
